...
check2:
  ...
  store i8 47, ptr %last_ptr  ; This makes sum divisible by 96
  
  %check2_result = call i1 @atlas.conserved.check(ptr %buf, i64 96)
  br i1 %check2_result, label %test_window, label %fail

test_window:
  ; Test 3: Window check should give same result as regular check for same buffer
  %window_check = call i1 @atlas.conserved.window.check(ptr %buf, i64 96)
  br i1 %window_check, label %test_window_invalid, label %fail

test_window_invalid:
  ; Test 4: Window check with null pointer should return false
  %null_check = call i1 @atlas.conserved.window.check(ptr null, i64 96)
  %null_ok = icmp eq i1 %null_check, false
  br i1 %null_ok, label %test_window_zero_len, label %fail

test_window_zero_len:
  ; Test 5: Window check with zero length should return false
  %zero_len_check = call i1 @atlas.conserved.window.check(ptr %buf, i64 0)
  %zero_len_ok = icmp eq i1 %zero_len_check, false
  br i1 %zero_len_ok, label %pass, label %fail
