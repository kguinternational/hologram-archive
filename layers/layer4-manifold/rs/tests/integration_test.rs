...
use atlas_manifold::{
    types::*, error::*, init, cleanup,
    manifold::{ManifoldAtlas, CoordinateChart},
    shard::{ShardManager, ShardStrategy, ShardMetadata},
    tlv::{TlvEncoder, TlvDecoder},
    coords::{CoordinateSystem, CoordinateTransform, util as coords_util},
    linear::util,
};
...
fn test_complete_manifold_workflow() {
    ...
    let domain_bounds = [(-std::f64::consts::PI, std::f64::consts::PI), 
...
fn test_shard_management_workflow() {
    ...
    let points = vec![
        ...
        AtlasPoint::<2> { coords: [-1.0, -1.0] },
        AtlasPoint::<2> { coords: [2.0, -2.0] },
        AtlasPoint::<2> { coords: [-3.0, 3.0] },
    ];
...
fn test_coordinate_transformation_workflow() {
    ...
    let cartesian_point = AtlasPoint::<3> { coords: [1.0, 0.0, 0.0] };
    ...
    for i in 0..3 {
        assert!(
            ...
            "Round-trip coordinate transformation failed at dimension {}", i
        );
    }
...
#[test]
#[ignore] // TODO: Fix TLV serialization format alignment issues
fn test_serialization_workflow() {
    ...
    let point = AtlasPoint::<4> { coords: [1.0, 2.0, 3.0, 4.0] };
    let vector = AtlasVector::<4> { components: [0.5, -0.5, 1.5, -1.5] };
    ...
    assert_eq!(decoded_desc.gaussian_curvature(), manifold_desc.gaussian_curvature());
...
fn test_manifold_geometry_workflow() {
    ...
    euclidean_atlas.add_chart(
        CoordinateChart::<2, 2>::identity_square(bounds_2d)
    );
...
fn test_error_recovery_workflow() {
    ...
    let outside_point = AtlasPoint::<2> { coords: [10.0, 10.0] };
    ...
    let strategy = ShardStrategy::Spatial { bounds: vec![(0.0, 1.0)] };
    let manager = ShardManager::new(strategy);
    
    // Try to shard a point with wrong dimensions
    let wrong_dim_point = AtlasPoint::<3> { coords: [0.5, 0.5, 0.5] };
...
#[test]
#[ignore] // TODO: Fix TLV serialization used in multithreaded test
fn test_multithreaded_safety() {
    ...
    let points = Arc::new(vec![
        AtlasPoint::<3> { coords: [1.0, 0.0, 0.0] },
        AtlasPoint::<3> { coords: [0.0, 1.0, 0.0] },
        AtlasPoint::<3> { coords: [0.0, 0.0, 1.0] },
        AtlasPoint::<3> { coords: [1.0, 1.0, 1.0] },
    ]);
