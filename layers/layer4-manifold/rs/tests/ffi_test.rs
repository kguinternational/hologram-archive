...
fn test_c_api_version_safety() {
    unsafe {
        ...
        assert_eq!(
            version_ptr, version_ptr2,
            "Version pointer should be stable"
        );
    }
}
...
fn test_c_api_initialization_safety() {
    ...
    assert_eq!(
        atlas_manifold_init(),
        0,
        "Third initialization should be safe"
    );
    ...
    assert_eq!(
        atlas_manifold_init(),
        0,
        "Initialization after cleanup should work"
    );
...
fn test_manifold_handle_null_safety() {
    ...
    unsafe {
        ...
        assert!(
            handle.is_null(),
            "Handle creation returns null (not implemented)"
        );
        ...
        assert!(
            handle.is_null(),
            "Creation with invalid dimensions should fail"
        );

        let handle = atlas_manifold_create(2, 0);
        assert!(
            handle.is_null(),
            "Creation with zero embedding dimension should fail"
        );
        ...
        assert!(
            handle.is_null(),
            "Creation with very large dimensions should fail"
        );
    }
...
fn test_linear_transform_null_safety() {
    ...
    unsafe {
        ...
        assert_eq!(
            result, 0,
            "Linear transform with valid parameters should succeed"
        );
    }
...
fn test_serialization_null_safety() {
    ...
    unsafe {
        ...
        assert_ne!(
            result, 0,
            "Serialization with null bytes_written should fail"
        );

        // Test deserialization with null buffer
        let mut handle_ptr: *mut AtlasManifoldHandle = ptr::null_mut();
        let result = atlas_manifold_deserialize(ptr::null(), 0, &mut handle_ptr);
        assert_ne!(result, 0, "Deserialization with null buffer should fail");

        // Test deserialization with null handle output
        let result = atlas_manifold_deserialize(buffer.as_ptr(), buffer.len(), ptr::null_mut());
        assert_ne!(
            result, 0,
            "Deserialization with null handle output should fail"
        );
    }
...
fn test_memory_boundary_conditions() {
    ...
    unsafe {
        ...
        assert_ne!(
            result, 0,
            "Transform with zero-dimensional input should fail"
        );
    }
    ...
    unsafe {
        ...
        assert_ne!(
            result, 0,
            "Transform with very large dimensions should fail safely"
        );
    }
...
fn test_ffi_parameter_validation() {
    ...
    unsafe {
        ...
        let result =
            atlas_manifold_transform_point(ptr::null(), &valid_point, &mut mismatched_output);
        assert_ne!(
            result, 0,
            "Transform with mismatched dimensions should fail"
        );
    }
