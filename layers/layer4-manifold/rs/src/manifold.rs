...
fn compute_spectral_moment<const N: usize>(matrix: &TransformMatrix<N, N>, k: u32) -> Float {
    match k {
        ...
        2 => {
            ...
            (0..N).map(|i| (0..N).map(|j| matrix.elements[i][j] * matrix.elements[j][i]).sum::<Float>()).sum()
        },
        3 => {
            // Tr(M^3) - simplified approximation for computational efficiency
            let trace = (0..N).map(|i| matrix.elements[i][i]).sum::<Float>();
            let trace_squared = (0..N).map(|i| (0..N).map(|j| matrix.elements[i][j] * matrix.elements[j][i]).sum::<Float>()).sum::<Float>();
...
fn compute_harmonic_coupling<const N: usize>(matrix: &TransformMatrix<N, N>, i: usize, j: usize) -> Float {
    if i >= N || j >= N {
        return 0.0;
    }
...
impl<const N: usize, const M: usize> ManifoldAtlas<N, M> {
    ...
    pub fn metric_tensor(&self, point: &AtlasPoint<N>) -> AtlasResult<TransformMatrix<N, N>> {
        ...
        for i in 0..N {
            ...
            for j in 0..N {
                if i != j {
                    metric.elements[i][j] = compute_harmonic_coupling(&coord_matrix, i, j) * norm_correction * 0.05;
                }
            }
        }
