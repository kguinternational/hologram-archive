...
impl<const FROM: usize, const TO: usize> CoordinateTransform<FROM, TO> {
    ...
    pub fn transform_point(&self, point: &AtlasPoint<FROM>) -> AtlasResult<AtlasPoint<TO>> {
        match (self.from_system, self.to_system) {
            ...
            (CoordinateSystem::Polar, CoordinateSystem::Cartesian) => {
                self.polar_to_cartesian(point)
            },
            (CoordinateSystem::Cartesian, CoordinateSystem::Homogeneous) => {
                self.cartesian_to_homogeneous(point)
            },
            (CoordinateSystem::Homogeneous, CoordinateSystem::Cartesian) => {
                self.homogeneous_to_cartesian(point)
            },
            _ => {
                Err(AtlasError::CoordinateError("unsupported coordinate transformation"))
            }
        }
    }
    ...
    fn cartesian_to_spherical(&self, point: &AtlasPoint<FROM>) -> AtlasResult<AtlasPoint<TO>> {
        if FROM < 2 || TO < 2 {
            return Err(AtlasError::InvalidDimension(core::cmp::min(FROM, TO) as u32));
        }
        ...
        result_coords[0] = r;
        if TO > 1 { result_coords[1] = theta; }
        if TO > 2 { result_coords[2] = phi; }
        
        // Copy remaining coordinates if any
        for i in 3..core::cmp::min(FROM, TO) {
            result_coords[i] = point.coords[i];
        }
        
        Ok(AtlasPoint::new(result_coords))
    }
    
    fn spherical_to_cartesian(&self, point: &AtlasPoint<FROM>) -> AtlasResult<AtlasPoint<TO>> {
        if FROM < 2 || TO < 2 {
            return Err(AtlasError::InvalidDimension(core::cmp::min(FROM, TO) as u32));
        }
        ...
        if TO > 1 { result_coords[1] = r * sin_theta * sin_phi; }
        if TO > 2 { result_coords[2] = r * cos_theta; }
    ...
    fn cartesian_to_cylindrical(&self, point: &AtlasPoint<FROM>) -> AtlasResult<AtlasPoint<TO>> {
        if FROM < 3 || TO < 3 {
            return Err(AtlasError::InvalidDimension(core::cmp::min(FROM, TO) as u32));
        }
    ...
    fn cylindrical_to_cartesian(&self, point: &AtlasPoint<FROM>) -> AtlasResult<AtlasPoint<TO>> {
        if FROM < 3 || TO < 3 {
            return Err(AtlasError::InvalidDimension(core::cmp::min(FROM, TO) as u32));
        }
    ...
    fn cartesian_to_polar(&self, point: &AtlasPoint<FROM>) -> AtlasResult<AtlasPoint<TO>> {
        if FROM < 2 || TO < 2 {
            return Err(AtlasError::InvalidDimension(core::cmp::min(FROM, TO) as u32));
        }
        ...
        // Copy remaining coordinates if any
        for i in 2..core::cmp::min(FROM, TO) {
            result_coords[i] = point.coords[i];
        }
    ...
    fn polar_to_cartesian(&self, point: &AtlasPoint<FROM>) -> AtlasResult<AtlasPoint<TO>> {
        if FROM < 2 || TO < 2 {
            return Err(AtlasError::InvalidDimension(core::cmp::min(FROM, TO) as u32));
        }
    ...
    fn cartesian_to_homogeneous(&self, point: &AtlasPoint<FROM>) -> AtlasResult<AtlasPoint<TO>> {
        if TO != FROM + 1 {
            return Err(AtlasError::InvalidDimension(TO as u32));
        }
    ...
    fn homogeneous_to_cartesian(&self, point: &AtlasPoint<FROM>) -> AtlasResult<AtlasPoint<TO>> {
        if FROM != TO + 1 {
            return Err(AtlasError::InvalidDimension(FROM as u32));
        }
        ...
        for i in 0..TO {
            result_coords[i] = point.coords[i] / w;
        }
