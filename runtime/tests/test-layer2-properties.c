...
void test_conservation_law_property(void) {
    ...
    for (int i = 0; i < PROPERTY_TEST_ITERATIONS; i++) {
        ...
        uint8_t expected_delta = (uint8_t)(i % 96);
        
        // Modify one byte to create the exact delta we want
        if (sizeof(after_buffer) > 0) {
            int change = (int)expected_delta - (int)after_buffer[0];
            after_buffer[0] = (uint8_t)((after_buffer[0] + change) % 256);
        }
        ...
        // The delta should match our expected delta modulo 96
        bool conservation_holds = (actual_delta == expected_delta);
